...
def get_camera_frustum_corners(camera, farplane_ratio=1.0, nearplane_ratio=0.0):
    ...
    You can specifiy the generated frustum via the farplane_ratio and nearplane_ratio parameters, 
    which are NOT the clipping distances but the wireframe visualization of the frustum in Blender.
    The camera in blender is visualized as a pyramid with the apex at the camera's origin (ratio=0) and the base at the cameras view_frame (ratio=1).
    A farplane_ratio=1.0 and nearplane_ratio=0.0 draws a frustum as visualized as the camera in Blender.
    ...
    list of mathutils.Vector
        The corners of the camera's frustum in world space, wheres always pairs of two corners represent a frustum edge connecting the near and far plane.
    """
...
#######################################################################################################################
def draw_camera_frustum(camera, farplane_ratio=1.0, nearplane_ratio=0.3, thickness=0.025):
    """ Draw a frustum pyramid for the given camera object. 
    You can specifiy the generated frustum via the farplane_ratio and nearplane_ratio parameters, 
    which are NOT the clipping distances but the wireframe visualization of the frustum in Blender.
    The camera in blender is visualized as a pyramid with the apex at the camera's origin (ratio=0) and the base at 
    the cameras view_frame (ratio=1). A farplane_ratio=1.0 and nearplane_ratio=0.0 draws a frustum as visualized as 
    the camera in Blender.
    
    Parameters
    ----------
    camera : bpy.types.Object
        The camera object for which the frustum pyramid will be drawn.
    farplane_ratio : float, optional
        The ratio of the far plane distance to the camera's far clipping distance (default is 1.0).
    nearplane_ratio : float, optional
        The ratio of the near plane distance to the camera's near clipping distance (default is 0.0).
    ...
    frustum_corners = get_camera_frustum_corners(camera, farplane_ratio, nearplane_ratio)
